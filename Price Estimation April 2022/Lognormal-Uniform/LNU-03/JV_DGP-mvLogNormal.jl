...
function sim_data_JV_LogNormal(β_up, β_down, Σ_up, Σ_down, n_firms,i, flag, obs_up, obs_down, d_min_prof_input)
    if flag == false
        ...
        up_data[1:2,:] = rand(Random.seed!(1234+i), MvLogNormal([0.; 0 ], Σ_up[1:2,1:2]),n_firms)
        up_data[3,:] = rand(Random.seed!(1234+i), Uniform(0,1),n_firms)
        
        down_data = Array{Float64, 2}(undef, 3, n_firms)
        
        down_data[1:2,:] = rand(Random.seed!(11234+i), MvLogNormal([0.; 0  ], Σ_down[1:2,1:2]),n_firms)
        down_data[3,:] = rand(Random.seed!(11234+i), Uniform(0,1),n_firms)
