#LOG NORMAL DGP
function sim_data_JV_LogNormal(β_up, β_down, Σ_up, Σ_down, n_firms,i, flag, obs_up, obs_down, d_min_prof_input, sel_mode="median")
    if flag == false
        ...
        up_data[3,:] = rand(Random.seed!(1234+3i), Uniform(0,1), n_firms)
        ...
        down_data[3,:] = rand(Random.seed!(1234+6i), Uniform(0,1), n_firms)

    elseif flag==true # take observed data as given (do not generate observed chars)
        
        up_data = Array{Float64, 2}(undef, 3, n_firms)
        up_data[1:2,:] = obs_up
        up_data[3,:] = rand(Random.seed!(1234+i), Uniform(0,1), n_firms)
...
function sim_data_JV_up_obs(β_up, β_down, Σ_up, Σ_down, n_firms,i, flag, obs_up, d_min_prof_input, sel_mode="median")
