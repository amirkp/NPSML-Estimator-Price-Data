...
#### Uniform distribution only two parameters
#### b33d and eq sel. unfiorm dist of unobs
using Distributed
using BSON
# using FLoops
...
for n_sim =50:300:50
    for n_firms in [50, 100]
        for data_mode =3:3
            est_pars = pmap(x->replicate_byseed(x, n_firms, n_sim, 9:10, 100*(n_firms/50)^2 ,
                     10, data_mode,[1., 1., 1]) ,1:48)
