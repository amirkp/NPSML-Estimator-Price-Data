...
@everywhere function replicate_byseed(n_rep, n_firms, n_sim, h_scale,  par_ind, sel_mode, globT, locT)
...
for n_sim =25:25:50
    for n_firms = 50:50:300
        est_pars = pmap(x->replicate_byseed(x, n_firms, n_sim, [1., 1., 1.], 9:10, "median", 60*(n_firms/50)^2 , 10*(n_firms/50)^2), 1:n_reps)
        estimation_result = Dict()
        push!(estimation_result, "beta_hat" => est_pars)
        bson("/home/ak68/rest-2p/est_$(n_firms)_sim_$(n_sim)_9-10.bson", estimation_result)
        # bson("/Users/akp/github/NPSML-Estimator-Price-Data/Price Estimation April 2022/NOTS/LogNormal/rest-2p/est_$(n_firms)_sim_$(n_sim)_mean.bson", estimation_result)
    end
end
