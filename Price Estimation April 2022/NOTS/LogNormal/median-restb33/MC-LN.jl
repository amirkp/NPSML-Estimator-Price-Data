...
@everywhere function replicate_byseed(n_rep, n_firms, n_sim, h_scale, sel_mode)
    ...
    bbo_ndim = 9
    bbo_feval = 50000
    fun = x->loglike(vcat(x[1:8], -3, x[9]))
...
for n_sim =50:25:50
    for n_firms = 50:100:150
        est_pars = pmap(x->replicate_byseed(x, n_firms, n_sim, [1., 1., 1.], "median"), 1:n_reps)
        estimation_result = Dict()
        push!(estimation_result, "beta_hat" => reduce(vcat, [est_pars[i][1:9] for i =1:n_reps]'))
        push!(estimation_result, "fitness" => reduce(vcat, [est_pars[i][10] for i =1:n_reps]'))
        push!(estimation_result, "bw" => reduce(vcat, [est_pars[i][11:13] for i =1:n_reps]'))
        bson("/home/ak68/median-restb33/est_$(n_firms)_sim_$(n_sim)_b33.bson", estimation_result)
