...
function sim_data_JV_LogNormal(β_up, β_down, Σ_up, Σ_down, n_firms,i, flag, obs_up, obs_down, d_min_prof_input)
    if flag == false
        ...
        up_data = rand(Random.seed!(1234+i), MvLogNormal([0.; 0; 0 ], Σ_up),n_firms)
        
        down_data = Array{Float64, 2}(undef, 3, n_firms)
        
        down_data = rand(Random.seed!(11234+i), MvLogNormal([0.; 0 ; 0 ], Σ_down),n_firms)
        
    elseif flag==true # take observed data as given (do not generate observed chars)
        
        up_data = Array{Float64, 2}(undef, 3, n_firms)
        up_data[1:2,:] = obs_up
        up_data[3,:] = rand(Random.seed!(1234+i), LogNormal(0., Σ_up[3,3]), n_firms)

        
        down_data = Array{Float64, 2}(undef, 3, n_firms)
        down_data[1:2,:] = obs_down
        down_data[3,:] = rand(Random.seed!(11234+i), LogNormal(0., Σ_down[3,3]), n_firms)
    end
