...
@everywhere begin 
    ...
    n_rep = 220; n_firms=50; n_sim =50; h_scale = [1., 1., 1.];
    data_mode=3
    xcor = 0.3
    x1var = .1
    x2var = .2
...
@everywhere function opt_instance(in)
# for i = 1:nopts
    ...
    return  vcat(Optim.minimizer(opt2), Optim.minimum(opt2))'
end

opt_mat = pmap(opt_instance, 1:96)
opt_mat1 = pmap(opt_instance, 1:96)
ests1= reduce(vcat, opt_mat1)
ests= reduce(vcat, opt_mat)

scatter(ests[:,2], ests[:,3])
scatter(ests1[:,2], ests1[:,3])

xv = -5.5:0.01:1.5
scatter(xv,[loglike(vcat(true_pars[1:8],xv[i],3)) for i=1:length(xv)], markersize=2, xlims = (-2.5,1.5),ylims=(-1,2))
scatter!(xv,[loglike(vcat(true_pars[1:8],xv[i],4.5)) for i=1:length(xv)], markersize=2)
hline!([minimum([loglike(vcat(true_pars[1:8],xv[i],1.5)) for i=1:length(xv)])])
