...
function sim_data_sinkhorn(β_up, β_down, Σ_up, Σ_down, n_firms,i)
    ...
    return up_data, down_match_data_exp, up_profit_data_sinkhorn_exp, down_match_profit_sinkhorn_exp
end
...
ud, dd, up, dp = sim_data_sinkhorn(b1,b2,s1,s2,1500,2)

scatter(ud[1,:], ud[2,:], markersize=2.)
scatter(ud[1,:], dd[1,:], markersize=2.)
scatter(ud[1,:],up, markersize=2.)
scatter(dd[1,:],dd[2,:], markersize=2.)

scatter(ud[1,:])
scatter(dd[1,:])
scatter(dat[:,1], dat[:,2], markersize=2)

dd1 = zeros(2, n_firms)
dd1[1,:] = rand(LogNormal(s2[1,1], s2[1,2]), n_firms)
dd1[2,:] = rand(LogNormal(Σ_down[2,1], Σ_down[2,2]), n_firms)
...
scatter(up_data[1,:],down_data[1,:], markersize=3)
scatter!(up_data[1,:],down_match_data_exp[1,:], markersize=2)
scatter!(up_data[1,:],down_match_data_maxrow[1,:], markersize=1)
...
scatter(up_data[1,:], up_profit_data_cf, markersize=3, color=:red)
scatter!(up_data[1,:], up_profit_data_sinkhorn, markersize=2., color=:yellow)


scatter(down_data[1,:], down_profit_data_cf, markersize=3, color=:red)
scatter!(down_data[1,:], down_match_profit_sinkhorn, markersize=2., color=:yellow)
