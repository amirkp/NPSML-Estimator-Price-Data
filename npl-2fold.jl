...
@everywhere n_firms=1000 #divide sample in two parts for two fold
...
@everywhere function replicate_byseed(n_rep)
    ...
    p_range = [(0.,5.),(3., 8.0),(-3.0, 3.0),(-3.0,3.0),(0.5,17.),(0.0005,7.)];
    res_global = bboptimize(like_5d_ucv; SearchRange = p_range, NumDimensions = 6,PopulationSize = 40, Method = :adaptive_de_rand_1_bin_radiuslimited, MaxTime =400.0)
    cand_global = best_candidate(res_global)
    if like_5d_ucv(cand_global)<Inf
        res = Optim.optimize(like_5d_ucv, cand_global, LBFGS(), Optim.Options(show_every = 1, time_limit=2500, show_trace = true,extended_trace=true) )
    else
        res = Optim.optimize(like_5d_ucv, true_pars+rand(5), LBFGS(), Optim.Options(show_every = 1, time_limit=2500, show_trace = true,extended_trace=true) )
    end
...
bson("/scratch/ak68/nphi/bcv_spec1_1500_1000_rand.bson", estimation_result)
