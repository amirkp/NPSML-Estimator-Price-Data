...
@everywhere function rep_fun(i)
    ...
    up_data, down_data, up_profit_data, down_profit_data, dd =
     sim_data_LP(A_mat, bx, by,  sig_up, sig_down, n_firms, 25+n_rep)
    ...
    # scatter(data[:,1], data[:,3], markersize=2, color=:yellow)
    ...
    # cv_int_y([1.2,0.2])
    res_y = Optim.optimize(cv_int_2d, [1.,1.])
...
#####################
    ...
    function md_objective(b)
        ...
        for j = 1:n_firms
            surplus_vec[j] += (b[5] *data[j,1] + b[6]* data[j,2] +  b[7]* data[j,3])
        end
    ...
    # res= Optim.optimize(md_objective, [3., 2.,1. ,.4,3])
    res = zeros(10, 8)
    for k =1:10
        @show init_p = 10*rand(7)
        opt = Optim.optimize(md_objective, init_p)
        res[k,1:7]= Optim.minimizer(opt)
        res[k,8]= Optim.minimum(opt)
    end



    # res= Optim.optimize(md_objective, -10,10)
    ...
    # println("replication ", n_rep, " parameters are ", Optim.minimizer(res), " lowest value ",Optim.minimum(res))
#
