...
up_data, down_data, up_profit_data_cf, down_profit_data_cf, price_data_cf, A_mat, β_diff, β_up, β_down, Σ_up, Σ_down =
 sim_data([2.,10.5, 1.], [1.5, 2., 1.], [3., 1., 1.], [2., 1., 3.], n_firms, 25)
...
up_data_2, down_data_2, up_profit_data_cf_2, down_profit_data_cf_2, price_data_cf_2, A_mat_2, β_diff_2, β_up_2, β_down_2, Σ_up_2, Σ_down_2 =
 sim_data_like(up_data[1,:],[minimizer_4d_1234[1],minimizer_4d_1234[2], 1.], [minimizer_4d_1234[3], minimizer_4d_1234[4], 1.], [3., 1., 1.], [2., 1., 3.], n_firms, 35)



p1 = scatter(up_data[1,:], down_data[1,:], markersize=2, xlabel="x", ylabel="y", title="Matching- true DGP", ylims=(-5,5))
p2 = scatter(up_data_2[1,:], down_data_2[1,:], markersize=2,  xlabel="x", ylabel="y", title="Matching- alt DGP",ylims=(-5,5))
p3 = scatter(up_data[1,:], price_data_cf, markersize=2,  xlabel="x", ylabel="p", title="Prices- true DGP", ylims=(-1700,30))
p4 = scatter(up_data[1,:], price_data_cf_2, markersize=2,  xlabel="x", ylabel="p", title="Prices- alt DGP",ylims=(-1700,30))


allplots = plot(p1,p2,p3,p4, layout=(2,2), legend=false)

mean(price_data_cf)
mean(price_data_cf_2)

using DataFrames, GLM

data1 = DataFrame(X1=up_data[1,:], Y1=down_data[1,:],P1 = price_data_cf)
data2 = DataFrame(X2=up_data_2[1,:], Y2=down_data_2[1,:], P2 = price_data_cf_2)


@show ols1= lm(@formula(Y1~X1),data1)

@show ols2= lm(@formula(Y2~X2),data2)
...
like_1d_1 = x-> loglikep([x, 1.5, 1.5, 2., 3.,4.], h_ucv)
plot(like_1d_1, -2., 5.)
...
res_1d2 = Optim.optimize(like_1d_2, -20., -.005)
@show minimizer_1d2 = Optim.minimizer(res_1d2)
@show min_1d2 = Optim.minimum(res_1d2)


like_2d_12 = x-> loglikep([x[1],x[2], 1.5, 2., 3.,3.], h_ucv)
...
Optim.optimize(like_5d_12, best_candidate(res_global), LBFGS(), Optim.Options(show_every = 1, time_limit=1000, show_trace = true, extended_trace=true) )
